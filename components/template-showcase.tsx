...
function CodeHighlighter({ code }: { code: string }) {
  ...
  return (
    <pre className="text-[10px] leading-[16px] font-mono text-fg-secondary p-4 bg-surface-raised overflow-auto h-full max-h-[350px] rounded-xl border border-border-base">
      {lines.map((line, idx) => (
        <div key={idx} className="flex">
          <span className="w-6 text-fg-muted text-right pr-2 select-none border-r border-border-base mr-2 text-[9px]">{idx + 1}</span>
...
function TemplateCard({ template, isActive, onSelect, onZoom, preview, isLoading, onLoadPreview }: TemplateCardProps) {
  ...
  const complexityColors = {
    Simple: 'bg-success-bg text-success border-success-border',
    Medium: 'bg-warning-bg text-warning border-warning-border',
    Advanced: 'bg-accent-muted text-accent border-accent-border'
  };
  ...
  return (
    <div className="group flex flex-col relative">
      ...
      <div 
        ...
        className={cn(
          "aspect-[16/11] bg-surface-raised relative overflow-hidden rounded-2xl border transition-all duration-300 cursor-pointer flex flex-col shadow-sm",
          isActive 
      ...
      >
        ...
        <div className="h-7 bg-surface border-b border-border-base px-3 flex items-center justify-between shrink-0">
          <div className="flex gap-1">
            <div className="w-1.5 h-1.5 rounded-full bg-border-strong" />
            <div className="w-1.5 h-1.5 rounded-full bg-border-strong" />
            <div className="w-1.5 h-1.5 rounded-full bg-border-strong" />
          </div>
          <div className="bg-surface-raised border border-border-base rounded px-2.5 py-0.5 text-[8px] text-fg-muted font-mono tracking-wide truncate max-w-[120px]">
            {template.id}.tsx
          </div>
          <div className="w-4" />
        </div>
        ...
        <div className="flex-1 bg-white relative overflow-hidden">
          ...
          <div className="absolute inset-0 bg-app/80 backdrop-blur-xs opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
            <button 
              onClick={(e) => {
                e.stopPropagation();
                onSelect(template);
              }}
              className="bg-accent hover:bg-accent-hover text-accent-fg px-4 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest shadow-md transform translate-y-2 group-hover:translate-y-0 transition-all duration-300 cursor-pointer"
            >
              Use Blueprint
            </button>
            <button
              ...
              className="bg-surface-raised hover:bg-surface-hover text-fg p-2 rounded-full shadow-md transform translate-y-2 group-hover:translate-y-0 transition-all duration-300 border border-border-base cursor-pointer"
          ...
          {isActive && (
            <div className="absolute top-2 right-2 bg-accent text-accent-fg p-1 rounded-full shadow-md z-10">
      ...
      <div className="mt-3.5 pl-0.5">
        <div className="flex items-start justify-between gap-2">
          <h4 
            onClick={() => onSelect(template)}
            className="font-bold text-[13px] text-fg tracking-tight hover:text-accent transition-colors cursor-pointer truncate"
          >
            {meta.name}
          </h4>
          <span className={cn(
        ...
        <div className="flex items-center gap-2 mt-1 text-[10px] text-fg-muted font-semibold uppercase tracking-wider">
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-surface w-full max-w-[95vw] xl:max-w-7xl h-full max-h-[88vh] rounded-[32px] shadow-2xl flex overflow-hidden border border-border-base text-fg-secondary"
      >
        
        {/* SIDEBAR CATEGORY EXPLORER */}
        <div className="w-[240px] bg-surface-raised border-r border-border-base p-6 flex flex-col justify-between shrink-0 hidden sm:flex">
          <div className="space-y-7">
            <div className="flex items-center gap-2">
              <Sliders className="w-4 h-4 text-accent" />
              <h3 className="font-bold text-xs tracking-wider uppercase text-fg">Blueprints</h3>
            </div>
              ...
              {[
                ...
                >
                  ...
                  <span className={cn(
                    ...
                    activeCategory === item.id
                      ? "bg-accent-hover text-accent-fg border-accent-border"
                      : "bg-surface text-fg-muted border-border-base"
                  )}>
          ...
          <div className="bg-surface border border-border-base rounded-2xl p-4 space-y-3.5">
            <div className="flex items-center gap-1.5 text-[9px] font-black uppercase text-fg-muted">
              <Info className="w-3 h-3 text-accent" /> COMPILER METRICS
            </div>
            <div className="space-y-2 text-[10px] font-semibold text-fg-muted">
              <div className="flex justify-between">
                <span>Output Format:</span>
                <span className="text-fg font-bold">Inlined CSS</span>
              </div>
              <div className="flex justify-between">
                <span>Size Control:</span>
                <span className="text-fg font-bold">102KB Warn</span>
              </div>
              <div className="flex justify-between">
                <span>Core Framework:</span>
                <span className="text-fg font-bold">React Email</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden bg-app">
          ...
          {/* Top Search Toolbar */}
          <div className="px-8 py-5 border-b border-border-base flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-surface shrink-0">
            <div>
              <h3 className="font-black text-xl text-fg tracking-tight leading-none">Select Blueprint</h3>
              <p className="text-[10px] text-fg-muted font-semibold uppercase tracking-wider mt-1.5">Open source React templates for inlining</p>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative w-full sm:w-56">
                <Input
                  ...
                  icon={<Search className="w-3.5 h-3.5 text-fg-muted" />}
                  className="h-9 bg-surface-raised rounded-full border-border-base focus-visible:ring-accent text-xs text-fg placeholder:text-fg-muted pl-9"
                />
              </div>

              <button
                onClick={onClose}
                className="w-8.5 h-8.5 rounded-full border border-border-base bg-surface-raised flex items-center justify-center text-fg-muted hover:text-fg transition-all hover:bg-surface-hover cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
          ...
          {/* Catalog grid body */}
          <div className="flex-grow overflow-y-auto px-8 py-8 bg-app custom-scrollbar animate-fade-in">
            ...
            {isGalleryLoading ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-8">
                {Array.from({ length: 6 }).map((_, idx) => (
                  <div key={idx} className="flex flex-col relative">
                    ...
                    <div className="aspect-[16/11] bg-surface-raised relative overflow-hidden rounded-2xl border border-border-base flex flex-col shadow-sm">
                      {/* Safari Top Bar Header */}
                      <div className="h-7 bg-surface border-b border-border-base px-3 flex items-center justify-between shrink-0">
                        <div className="flex gap-1">
                          <div className="w-1.5 h-1.5 rounded-full bg-border-strong" />
                        ...
                        <div className="bg-surface-raised border border-border-base rounded px-3 py-0.5 text-[8px] h-3.5 w-16 animate-pulse" />
                        <div className="w-4" />
                      </div>
                      {/* Viewport Frame Skeleton */}
                      <div className="flex-1 bg-surface relative overflow-hidden p-4 flex flex-col justify-between">
                        <div className="space-y-2">
                          <div className="h-2 w-1/4 bg-border-base rounded animate-pulse" />
                          <div className="h-3 w-5/6 bg-border-base rounded animate-pulse" />
                          <div className="h-2 w-full bg-border-base rounded animate-pulse" />
                        </div>
                        <div className="h-6 w-1/3 bg-border-base rounded-lg animate-pulse" />
                      </div>
                    </div>
                    {/* Metadata line underneath */}
                    <div className="mt-3.5 pl-0.5 space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="h-4 w-2/3 bg-border-base rounded animate-pulse" />
                        <div className="h-4 w-12 bg-border-base rounded animate-pulse" />
                      </div>
                      <div className="h-3 w-1/3 bg-border-base rounded animate-pulse" />
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="flex flex-col items-center justify-center py-28 text-center">
                <div className="w-12 h-12 bg-surface-raised rounded-2xl border border-border-base flex items-center justify-center mb-4 text-fg-muted shadow-sm">
                  <Inbox className="w-5 h-5" />
                </div>
                <h4 className="text-sm font-bold text-fg mb-1">No blueprints matched</h4>
                <p className="text-xs text-fg-muted font-medium max-w-xs leading-relaxed">
      ...
      <AnimatePresence>
        {zoomTemplate && activeZoomMeta && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[200] bg-black/75 backdrop-blur-md flex items-center justify-center p-4 md:p-6"
          >
            <motion.div
              initial={{ scale: 0.98, opacity: 0, y: 12 }}
              animate={{ scale: 1, opacity: 1, y: 0 }}
              exit={{ scale: 0.98, opacity: 0, y: 12 }}
              className="bg-surface w-full max-w-5xl h-full max-h-[82vh] rounded-[28px] overflow-hidden flex flex-col shadow-2xl border border-border-base text-fg-secondary"
            >
              ...
              <div className="px-6 py-4 border-b border-border-base flex justify-between items-center bg-surface shrink-0">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-surface-raised border border-border-base flex items-center justify-center text-accent shadow-xs">
                    <Sliders className="w-4 h-4" />
                  </div>
                  <div>
                    <h4 className="font-bold text-xs text-fg uppercase tracking-wide leading-none">
                      {activeZoomMeta.name}
                    </h4>
                    <span className="text-[8px] text-fg-muted font-black tracking-wider block mt-1.5 uppercase">Interactive Testing Sandbox</span>
                  </div>
                </div>

                <div className="flex items-center gap-3">
                  {/* Viewport Width toggle controls */}
                  <div className="flex bg-surface-raised border border-border-base rounded-xl p-0.5 shadow-xs">
                    {[
                      ...
                      <button
                        ...
                        className={cn(
                          "px-3 py-1.5 rounded-lg transition-all text-[8px] font-black uppercase tracking-wider flex items-center gap-1 cursor-pointer",
                          zoomPreviewMode === btn.id 
                            ? "bg-surface text-fg shadow-sm border border-border-base" 
                  ...
                  <Button
                    ...
                    className={cn(
                      ...
                      activeTemplateId === zoomTemplate.id 
                        ? "bg-surface-raised text-fg-muted border border-border-base cursor-not-allowed shadow-none" 
                  ...
                  <button
                    ...
                    className="w-8.5 h-8.5 rounded-full border border-border-base bg-surface-raised text-fg-muted hover:text-fg flex items-center justify-center p-0 transition-colors cursor-pointer"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ...
              <div className="flex-1 flex overflow-hidden bg-app">
                ...
                <div className="flex-1 flex items-center justify-center p-6 md:p-8 overflow-hidden relative border-r border-border-base">
                  <div 
                    className={cn(
                      "h-full bg-white shadow-md transition-all duration-300 rounded-2xl overflow-hidden border border-border-base flex flex-col",
                ...
                <div className="w-[320px] bg-surface flex flex-col justify-between shrink-0 p-5 overflow-y-auto custom-scrollbar border-l border-border-base">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex bg-surface-raised border border-border-base rounded-xl p-0.5 shadow-xs">
                      <button
                        ...
                        className={cn(
                          "flex-1 py-1.5 rounded-lg transition-all text-[9px] font-black uppercase tracking-wider flex items-center justify-center gap-1.5 cursor-pointer",
                          zoomTab === 'preview' ? "bg-surface text-fg shadow-sm border border-border-base" : "text-fg-muted hover:text-fg"
                        )}
                      >
                      ...
                      <button
                        ...
                        className={cn(
                          "flex-1 py-1.5 rounded-lg transition-all text-[9px] font-black uppercase tracking-wider flex items-center justify-center gap-1.5 cursor-pointer",
                          zoomTab === 'code' ? "bg-surface text-fg shadow-sm border border-border-base" : "text-fg-muted hover:text-fg"
                        )}
                      >
                    ...
                    {zoomTab === 'preview' ? (
                      ...
                      <div className="space-y-5">
                        <div>
                          <span className="text-[8px] font-black uppercase tracking-widest text-fg-muted block mb-1">Blueprint Profile</span>
                          <p className="text-xs text-fg-secondary font-medium leading-relaxed">
                            {activeZoomMeta.description}
                          </p>
                        </div>

                        <div className="bg-surface-raised border border-border-base rounded-2xl p-4 space-y-3 shadow-xs">
                          <span className="text-[8px] font-black uppercase tracking-widest text-fg-muted block">Accessibility Audits</span>
                          ...
                          <div className="space-y-2">
                            {[
                              ...
                              <div key={audit.label} className="flex items-start gap-2.5">
                                ...
                                <div>
                                  <div className="text-[10px] font-bold text-fg leading-none">{audit.label}</div>
                                  <span className="text-[8px] text-fg-muted font-medium">{audit.desc}</span>
                                </div>
                              </div>
                            ))}
                          </div>
                        ...
                        <div className="space-y-2 text-[10px] font-semibold text-fg-muted pl-0.5">
                          <div className="flex justify-between">
                            <span>Complexity:</span>
                            <span className="text-fg font-bold">{activeZoomMeta.complexity}</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Language:</span>
                            <span className="text-fg font-bold uppercase">{zoomTemplate.language}</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Estimated Read Time:</span>
                            <span className="text-fg font-bold">{activeZoomMeta.readTime}</span>
                          </div>
                        </div>
                      </div>
                    ) : (
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="text-[8px] font-black uppercase tracking-widest text-fg-muted flex items-center gap-1.5"><FileCode className="w-3.5 h-3.5" /> {zoomTemplate.id}.tsx</span>
                          <button
                            onClick={() => handleCopyCode(zoomTemplate.code)}
                            className="text-[9px] font-black uppercase tracking-wider text-accent hover:text-accent-hover flex items-center gap-1 transition-colors cursor-pointer"
                          >
                  ...
                  <div className="pt-4 border-t border-border-base mt-4 flex items-center gap-2 text-[9px] font-bold text-fg-muted">

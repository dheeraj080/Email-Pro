...
export function ThemePicker({ currentCode, onCodeChange }: ThemePickerProps) {
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 bottom-full mb-1.5 w-44 bg-surface border border-border-base rounded-xl shadow-2xl p-3.5 z-[200] animate-in fade-in slide-in-from-bottom-1 duration-200">
          <div className="text-[8px] font-black uppercase tracking-wider text-fg-muted mb-2">Accent Syncer</div>
              ...
              <button
                ...
                className={cn(
                  ...
                  activeThemeName === theme.name 
                    ...
                    : "border-border-base bg-surface-raised hover:border-border-strong"
                )}
                title={theme.label}
              >
                <div 
                ...
                />
                <span className="text-[7px] font-bold text-fg-muted mt-1 uppercase tracking-wide">{theme.name}</span>
                {activeThemeName === theme.name && (
                  <div className="absolute -top-1.5 -right-1.5 bg-accent text-accent-fg rounded-full p-0.5 shadow-sm">
